...
public class BlockingResponseHandler<T extends Response> implements ResponseHandler<T> {
    ...
    public BlockingResponseHandler(final ObjectReader objectReader, final Function<Exception, T> fallbackResponseTemplate) {
        this.deserializer = new JacksonDeserializer<>(objectReader);
    ...
    /**
     * Sets the result. Releases all threads waiting at {@link #getResult()}
     *
     * @param result result
     */
    @Override
    public final void completed(final T result) {
        this.result = result;
     ...
     */
    @Override
    public final void failed(final Exception exception) {
        this.result = fallbackResponseTemplate.apply(exception);
        countDownLatch.countDown();
    }

    @Override
    public T deserializeResponse(final InputStream inputStream) throws IOException {
        if (inputStream == null) {
